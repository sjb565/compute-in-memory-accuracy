...
class PDF:
    ...
    def get_expected_value(self, func=lambda x: x, trapezoidal=True):
        """
        ...
        if isinstance(func, AddNoise):
            return (
                self.get_expected_value(func.noise_1, trapezoidal) +  
                self.get_expected_value(func.noise_2, trapezoidal)
            )
        elif isinstance(func, ReluNoise):
            func_value = self.get_expected_value(func.noise, trapezoidal)
            non_pos_prob = self.get_non_pos_prob()
            
            return func_value * (1-non_pos_prob)
...
class AddNoise(Noise):
    def __init__(self, noise_1, noise_2):
        """
        Sum of two noises
        """
        super().__init__()
    ...
    def __call__(self, x):
        if isinstance(self.var, torch.Tensor):
            ...
            if var.shape == x.shape[-self.var.dim():]: 
                #V ariance shape must match input shape
                out_shape = x.shape
