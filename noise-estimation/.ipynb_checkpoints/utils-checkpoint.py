...
from correlation_handler import get_spatial_correlation
...
from noise import Noise, PDF, GaussianNoise
...
def add_scale_to_analog_params(params):
    ...
    # get input, weight scale depending on number of input bits (if not externally specified)
    if input_bits and (params['input_scale'] is None):
    ...
    if weight_bits and (params['weight_scale'] is None):
...
def single_layer_conv2d_noise(
    ...
    print(input_pdf.pdf.shape, padding)
    # Get Dimensions
    ...
    if input_bits:
        ...
        input_var_total += input_pdf.get_expected_value(GaussianNoise(input_scale**2 / 12))

    if input_noise:
        # Add padding since Gaussian Noise Tensor size may differ from padded input
        input_noise.add_padding(padding)
        input_var_total += input_pdf.get_expected_value(input_noise)
        
    if input_var_total is not 0:
        input_val_noise_est = F.conv2d(input_var_total, weight_square, bias=None, stride=stride)
...
# N_Row, Input_Bit, Weight_Bit, Input_Slice, Weight_Slice, ADC Reso
# Differential / Offset
# Proportional / Independent Noise
# Max Pool, Avg Pool, Strided


# def analog_matrix_vector_multiply(
#         input_pdf: PDF, 
#         input_noise: Noise, 
#         weight: torch.Tensor, 
#         weight_noise: Noise):
#     """
#     Perform analog matrix-vector multiplication with noise.
#     Parameters:
#     - input_pdf (PDF): input PDF with range shape (n_bin, (*, n_row)).
#     - input_noise (Noise): input noise object.
#     - weight (*, n_row, n_col): weight values.
#     - weight_noise (Noise): weight noise object.
#     Returns:
#     - Noise: output noise of shape (*, n_col).
#     """
#     # Calculate the expected value of the product of the PDFs
#     input_var = input_pdf.get_expected_value(input_noise)
#     assert input_var.shape == weight.shape[:-1], "input_var shape should be equal to weight shape without last dimension"
    
#     noise_from_input_var = torch.einsum('...i,...ij->...j', input_var, weight**2)

#     # Noise from weight
#     weight_var = weight_noise(weight)
#     input_square_average = input_pdf.get_expected_value(lambda x: x**2)
#     noise_from_weight_var = torch.einsum('...i,...ij->...j', input_square_average, weight_var)

#     return noise_from_input_var + noise_from_weight_var

# def accumulate_sliced_noise(
#         noise_list_lsb_to_msb: list[torch.Tensor] | torch.Tensor,
#         bits_lsb_to_msb: list[int] | int
#     ) -> torch.Tensor:
#     """
#     Accumulate sliced noise from LSB to MSB.
#     If bits_lsb_to_msb is integer, assume all slices are using the same number of bits.
#     Parameters:
#     - noise_list_lsb_to_msb (list[torch.Tensor] | torch.Tensor): List of noise objects or a single noise object.
#     """
#     # If not sliced (noise object or length == 1), return original noise
#     if not isinstance(noise_list_lsb_to_msb, list):
#         return noise_list_lsb_to_msb
#     if len(noise_list_lsb_to_msb) == 1:
#         return noise_list_lsb_to_msb[0]

#     if isinstance(bits_lsb_to_msb, list):
#         assert len(noise_list_lsb_to_msb) == len(bits_lsb_to_msb), "Length of noise_list and bits_lsb_to_msb should be equal"
#     else:
#         bits_lsb_to_msb = [bits_lsb_to_msb] * len(noise_list_lsb_to_msb)
    
#     cur_scale = (1 << bits_lsb_to_msb[0])
#     cur_noise = noise_list_lsb_to_msb[0]
    
#     for noise, slice_bits in zip(noise_list_lsb_to_msb[1:], bits_lsb_to_msb[1:]):
#         cur_noise = cur_noise + (noise * (cur_scale**2))
#         cur_scale = (cur_scale << slice_bits)
