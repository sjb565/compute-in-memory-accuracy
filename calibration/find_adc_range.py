...
def find_adc_range(params_args, n_layers, manual, name):
    """
    ...
    if manual:
        return np.load(limits_dir+name) 
    else:
        if adc_bits > 0 and adc_range_option == "CALIBRATED":
            ...
            if Nslices == 1:
                if style == "BALANCED" and NrowsMax >= 576 and not adc_per_ibit and subtract_current_in_xbar:
                    adc_ranges = np.load(limits_dir+"adc_limits_ResNet18_balanced.npy")

                elif style == "BALANCED" and NrowsMax >= 576 and adc_per_ibit and subtract_current_in_xbar:
                    adc_ranges = np.load(limits_dir+"adc_limits_ResNet18_balanced_ibits.npy")

                elif style == "OFFSET" and NrowsMax >= 576 and adc_per_ibit:
                    adc_ranges = np.load(limits_dir+"adc_limits_ResNet18_offset.npy")
            else:
